...
test('los catálogos territoriales se importan, versionan y cachean con gobierno LATAM', async () => {
  const migration = await read('supabase/migrations/202608310008_versioned_jurisdiction_catalogs.sql')
  const catalog = await read('src/lib/jurisdiction-catalog.ts')
  const page = await read('src/pages/admin/CountryCatalogPage.tsx')
  const routes = await read('src/App.tsx')
  const sidebar = await read('src/components/layout/Sidebar.tsx')
  const gis = await read('src/lib/gis-service.ts')
  const mapPage = await read('src/pages/shared/OperationalMapPage.tsx')
  const verifier = await read('scripts/verify-jurisdiction-catalog.mjs')
  const bolivarPublisher = await read('scripts/publish-bolivar-jurisdictions.mjs')

  assert.match(migration, /create table if not exists public\.jurisdiction_import_runs/)
  assert.match(migration, /create or replace function public\.import_jurisdiction_catalog/)
  assert.match(migration, /security definer/)
  assert.match(migration, /Solo el superadministrador puede versionar/)
  assert.match(migration, /feature_count < 1 or feature_count > 10000/)
  assert.match(migration, /extensions\.st_geomfromgeojson/)
  assert.match(migration, /extensions\.st_makevalid/)
  assert.match(migration, /input_hash := encode\(extensions\.digest/)
  assert.match(migration, /set status = 'retired' where id = base_profile\.id/)
  assert.match(migration, /catalog_previous_parent_id/)
  assert.match(migration, /p_assign_active_entities/)
  assert.match(migration, /control_g\.bulk_jurisdiction_import/)
  assert.match(catalog, /MAX_FEATURES = 10_000/)
  assert.match(catalog, /POLYGON_TYPES = new Set\(\['Polygon', 'MultiPolygon'\]\)/)
  assert.match(catalog, /buildJurisdictionRecords/)
  assert.match(catalog, /import_jurisdiction_catalog/)
  assert.match(page, /Prevalidar sin publicar/)
  assert.match(page, /PUBLICAR \$\{countryCode\} V\$\{preview\.next_version\}/)
  assert.match(page, /Actualizar entidades activas/)
  assert.match(routes, /path="territories" element=\{<CountryCatalogPage \/>\}/)
  assert.match(sidebar, /\/admin\/territories/)
  assert.match(gis, /loadJurisdictionDocuments/)
  assert.match(gis, /jurisdictionLayers/)
  assert.match(gis, /COLLECTION_IDS\.JURISDICTIONS/)
  assert.match(gis, /cacheDataset\(scope, records, layers\)/)
  assert.match(mapPage, /Alcance multiempresa/)
  assert.match(mapPage, /cg_admin_map_entity/)
  assert.match(mapPage, /loadMapDataset\(scopedUser\)/)
  assert.match(verifier, /rollback;/)
  assert.match(verifier, /valid_geometries/)
  assert.match(bolivarPublisher, /geojson\.features\.length !== 46/)
  assert.match(bolivarPublisher, /where id = 'gov-bolivar-2026'/)
})
...
test('ArcGIS opera por cola segura y GeoPackage genera un archivo OGC offline', async () => {
  ...
  const attachmentMigration = await read('supabase/migrations/202608310009_arcgis_attachment_governance.sql')
  const workerMigration = await read('supabase/migrations/202608310010_arcgis_worker_leases.sql')
  const api = await read('api/arcgis/job.mjs')
  const attachments = await read('api/arcgis/attachments.mjs')
  const worker = await read('api/maintenance/arcgis.mjs')
  ...
  assert.match(attachmentMigration, /attachment_authorized_by uuid references auth\.users/)
  assert.match(attachmentMigration, /evidence_file_id text references public\.evidence_files/)
  assert.match(attachmentMigration, /arcgis_job_items_attachment_evidence_idx/)
  assert.match(attachmentMigration, /validate constraint arcgis_mapping_attachment_authorization_check/)
  assert.match(attachmentMigration, /operation = 'attachment'/)
  assert.match(workerMigration, /claim_due_arcgis_jobs/)
  assert.match(workerMigration, /skip locked/)
  assert.match(workerMigration, /lease_expires_at/)
  ...
  assert.match(api, /addAttachment/)
  assert.match(api, /multipart|FormData/)
  assert.match(api, /remoteAttachmentByName/)
  assert.match(api, /client\.storage\.from\(evidence\.bucket_id\)\.download/)
  assert.doesNotMatch(api, /service[_-]?role/i)
  assert.match(worker, /CRON_SECRET/)
  assert.match(worker, /SUPABASE_SERVICE_ROLE_KEY/)
  assert.match(attachments, /MAX_ATTACHMENTS_PER_FEATURE = 3/)
  assert.match(attachments, /media_type !== 'photo'/)
  assert.match(attachments, /ATTACHMENT_HASH_MISMATCH/)
  assert.match(attachments, /ATTACHMENT_CONTENT_MISMATCH/)
  assert.doesNotMatch(attachments, /signatures.*allowed/i)
  ...
  assert.match(page, /Autorizo publicar fotografías de evidencia en ArcGIS/)
  assert.match(page, /Control G excluye firmas/)
...
test('los indicadores se fijan en servidor y la retención se ejecuta con vista previa y auditoría', async () => {
  const migration = await read('supabase/migrations/202608310007_server_snapshots_and_retention.sql')
  const backend = await read('src/lib/backend.ts')
  const analytics = await read('src/pages/shared/AnalyticsPage.tsx')
  const governance = await read('src/pages/shared/GovernancePage.tsx')
  const cron = await read('api/maintenance/snapshots.mjs')
  const vercel = await read('vercel.json')
  const verify = await read('scripts/verify-supabase.mjs')

  assert.match(migration, /create or replace function public\.run_indicator_snapshots/)
  assert.match(migration, /control-g-server-v1/)
  assert.match(migration, /create table if not exists public\.retention_runs/)
  assert.match(migration, /p_execute boolean default false/)
  assert.match(migration, /RETENTION_CONFIRMATION_INVALID/)
  assert.match(migration, /requires_manual_workflow/)
  assert.match(migration, /public\.anonymize_response_json/)
  assert.match(backend, /runSnapshots/)
  assert.match(backend, /runRetentionPolicy/)
  assert.match(analytics, /Último corte fijado en servidor/)
  assert.match(governance, /Vista previa/)
  assert.match(governance, /Ejecución controlada/)
  assert.match(cron, /CRON_SECRET/)
  assert.match(cron, /SUPABASE_SERVICE_ROLE_KEY/)
  assert.match(vercel, /api\/maintenance\/snapshots/)
  assert.match(verify, /snapshots reproducibles/)
  assert.match(verify, /retención auditada/)
})

test('la navegación principal cumple una base accesible en web y APK', async () => {
  const app = await read('src/App.tsx')
  const skip = await read('src/components/a11y/SkipLink.tsx')
  const login = await read('src/pages/auth/LoginPage.tsx')
  const sidebar = await read('src/components/layout/Sidebar.tsx')
  const landing = await read('src/pages/landing/LandingPage.tsx')
  const solution = await read('src/pages/landing/SolutionPage.tsx')
  ...
  assert.match(skip, /Saltar al contenido principal/)
  assert.match(app, /<SkipLink \/>/)
  assert.match(app, /id="main-content"/)
  for (const source of [login, landing, solution, blogIndex, blogPost]) assert.match(source, /<main id="main-content" tabIndex=\{-1\}/)
  assert.match(login, /aria-label=\{showPassword \? 'Ocultar contraseña' : 'Mostrar contraseña'\}/)
  ...
  assert.match(login, /flex h-11 w-11/)
  assert.match(login, /role="alert"/)
  assert.match(sidebar, /aria-label="Abrir menú de navegación"/)
...
test('la operación incorpora health check real, telemetría mínima y recuperación segura', async () => {
  ...
  const uptimeWorkflow = await read('.github/workflows/production-health.yml')
  const uptimeProbe = await read('scripts/probe-production.mjs')
  const uptimeIncident = await read('scripts/manage-uptime-incident.mjs')
  ...
  assert.match(pkg.scripts['health:probe'], /probe-production/)
  ...
  assert.match(uptimeWorkflow, /issues: write/)
  assert.match(uptimeWorkflow, /continue-on-error: true/)
  assert.match(uptimeProbe, /checks\?\.database\?\.status === 'ok'/)
  assert.match(uptimeProbe, /method: 'HEAD'/)
  assert.match(uptimeIncident, /previous\[0\] !== 'failure'/)
  assert.match(uptimeIncident, /previous\[1\] === 'success'/)
  assert.doesNotMatch(uptimeWorkflow, /SUPABASE_.*KEY/)
})
