...
import { CheckCircle2, CloudCog, Loader2, PauseCircle, Play, Plus, RefreshCw, ShieldCheck, XCircle } from 'lucide-react'
...
export default function ArcGisIntegrationsPage() {
  ...
  const verify = async (connection: ArcGisConnectionRecord, mapping: ArcGisMappingRecord) => {
    ...
    try {
      ...
      setMessage(`Conexión verificada: ${result.summary?.name || connection.name}.`)
      await load()
    } catch (verifyError) {
  ...
  const run = async (mapping: ArcGisMappingRecord) => {
    ...
    try {
      ...
      setMessage(`Trabajo ${mapping.direction === 'import' ? 'de importación' : 'de exportación'} terminado: ${result.succeeded || 0} registros correctos, ${result.failed || 0} con error.`)
      await load()
    } catch (runError) {
