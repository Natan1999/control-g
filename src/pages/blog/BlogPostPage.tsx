...
import { blogCover, blogCoverAlt, formatBlogDate, relatedBlogPosts } from '@/lib/blog'
import { usePublishedBlogPost } from '@/hooks/useBlogContent'

export default function BlogPostPage() {
  const { slug = '' } = useParams()
  const { post, loading } = usePublishedBlogPost(slug)
  if (loading && !post) return <div className="min-h-screen bg-white"><div className="mx-auto h-[72px] max-w-7xl animate-pulse border-b border-slate-100" /><div className="h-[520px] animate-pulse bg-[#102d3e]" /></div>
      ...
      <main id="main-content" tabIndex={-1}>
        <article>
          <header className="bg-gradient-to-br from-[#102d3e] via-[#1B3A4B] to-[#2C6E8A] px-5 py-12 text-white sm:py-16">
            <div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-[minmax(0,1.05fr)_minmax(360px,.95fr)] lg:items-center">
              <div>
                ...
                <div className="mt-7 flex flex-wrap gap-2 text-xs font-black uppercase tracking-[.13em]"><span className="rounded-full bg-white/10 px-3 py-1.5 text-teal-200">{post.category}</span><span className="rounded-full bg-white/10 px-3 py-1.5 text-white/70">{post.country}</span></div>
                ...
                <div className="mt-7 flex flex-wrap items-center gap-x-5 gap-y-2 text-sm text-white/55"><span>Equipo editorial Control G</span><time dateTime={post.updatedAt}>Actualizado {formatBlogDate(post.updatedAt)}</time><span className="inline-flex items-center gap-1"><Clock3 size={15} /> {post.readingMinutes} min</span><span className="inline-flex items-center gap-1"><MapPin size={15} /> {post.country}</span></div>
              </div>
              <figure className="overflow-hidden rounded-3xl bg-white/10 shadow-2xl shadow-black/20 ring-1 ring-white/15"><img src={blogCover(post)} alt={blogCoverAlt(post)} className="aspect-[3/2] h-full w-full object-cover" width="1440" height="960" fetchPriority="high" /><figcaption className="sr-only">{blogCoverAlt(post)}</figcaption></figure>
            </div>
          </header>
          ...
          <div className="mx-auto grid max-w-6xl gap-12 px-5 py-14 lg:grid-cols-[minmax(0,760px)_280px] lg:items-start">
            ...
            <aside className="hidden lg:block lg:sticky lg:top-24"><div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-lg shadow-slate-900/5"><p className="text-xs font-black uppercase tracking-[.16em] text-[#2C6E8A]">En este artículo</p><nav className="mt-4 space-y-3 text-sm">{post.sections.map((section, index) => <a key={section.title} href={`#seccion-${index + 1}`} className="block leading-5 text-slate-600 hover:text-[#2C6E8A]">{section.title}</a>)}{post.comparison.length > 0 && <span className="block text-slate-600">Comparación funcional</span>}</nav></div></aside>
          </div>
        </article>

        <section className="bg-slate-50 px-5 py-16"><div className="mx-auto max-w-6xl"><h2 className="text-3xl font-black text-[#102d3e]">También puede interesarle</h2><div className="mt-7 grid gap-5 md:grid-cols-3">{related.map(item => <article key={item.slug} className="group overflow-hidden rounded-2xl border border-slate-200 bg-white"><Link to={`/blog/${item.slug}`} className="block h-40 overflow-hidden"><img src={blogCover(item)} alt="" className="h-full w-full object-cover transition duration-500 group-hover:scale-105" loading="lazy" /></Link><div className="p-6"><span className="text-xs font-black uppercase tracking-widest text-[#2C6E8A]">{item.category}</span><h3 className="mt-3 text-lg font-black leading-snug"><Link to={`/blog/${item.slug}`} className="hover:text-[#2C6E8A]">{item.title}</Link></h3><p className="mt-3 text-sm leading-6 text-slate-600">{item.excerpt}</p><Link to={`/blog/${item.slug}`} className="mt-4 inline-flex items-center gap-1 text-sm font-black text-green-700">Leer artículo <ArrowRight size={14} /></Link></div></article>)}</div><div className="mt-12"><LeadFunnel source={`articulo-${post.slug}`} /></div></div></section>
      </main>
