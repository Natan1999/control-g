...
2. Ejecuta, en orden, las migraciones de `supabase/migrations/` en la instancia de Supabase. Las migraciones `202608310001` a `202608310004` agregan PostGIS/GIS LATAM, gobierno de datos, evidencias con mínimo privilegio y versiones inmutables de formularios.
...
El APK instalable de pruebas queda en `android/app/build/outputs/apk/debug/app-debug.apk`; la copia entregable 2.4.0 se genera en `entregables/Control-G-2.4.0-LATAM-GIS-offline-debug.apk`. Para Play Store o distribución firmada se debe aportar el keystore institucional y configurar la firma de `release` fuera del repositorio.
